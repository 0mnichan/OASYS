  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: size * 0.21, fontWeight: 700, fontFamily: 'var(--font-mono)', color, lineHeight: 1, textShadow: `0 0 20px ${color}77` }}>
          {pct.toFixed(1)}
        </span>
        <span style={{ fontSize: size * 0.1, fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary)', marginTop: 2 }}>%</span>
      </div>
    </div>
  );
};
...
/* ─── 3D tilt ─── */
const Tilt = ({ children, style, onClick }: { children: React.ReactNode; style?: React.CSSProperties; onClick?: () => void }) => {
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} onClick={onClick}
      style={{ cursor: onClick ? 'pointer' : 'default', boxShadow: 'var(--shadow-md), 0 0 0 1px var(--border-card)', willChange: 'transform', ...style }}>
      {children}
    </div>
  );
};
...
const Bar = ({ pct, delay = 0 }: { pct: number; delay?: number }) => {
  ...
  return (
    <div style={{ height: 3, borderRadius: 99, background: 'var(--border-subtle)', overflow: 'hidden', marginTop: 12 }}>
      <div style={{ height: '100%', width: `${Math.min(w, 100)}%`, borderRadius: 99, background: color, boxShadow: `0 0 8px ${color}`, transition: `width 1s ${delay}ms cubic-bezier(.4,0,.2,1)` }} />
    </div>
  );
};
...
const StatTile = ({ label, value, color, sub, delay = 0 }: {
  ...
  return (
    <Tilt style={{
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700, color: 'var(--text-label)', letterSpacing: '0.13em', marginBottom: 10, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {label}
      </div>
      <div style={{ fontSize: 'clamp(20px,3vw,30px)', fontWeight: 700, fontFamily: 'var(--font-display)', color, lineHeight: 1, textShadow: `0 0 20px ${color}44` }}>
        {value}
      </div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-tertiary)', marginTop: 6 }}>
        {sub}
      </div>
    </Tilt>
  );
...
const CourseCard = ({ course, index, onClick }: { course: Course; index: number; onClick: () => void }) => {
  ...
  return (
    <Tilt onClick={onClick} style={{
      ...
      border: '1px solid var(--border-card)', borderRadius: 16, padding: '20px 22px',
      opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(20px)',
      transition: `opacity 0.45s ease, transform 0.45s ease`,
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-tertiary)', letterSpacing: '0.08em' }}>{course.code}</span>
        <span style={{ fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-mono)', color, lineHeight: 1, textShadow: `0 0 12px ${color}55` }}>{pct.toFixed(1)}%</span>
      </div>
      <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--text-primary)', lineHeight: 1.55, minHeight: 36 }}>{course.name}</div>
      <Bar pct={pct} delay={80 + index * 50} />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14 }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-tertiary)' }}>{course.attended}/{course.total}</span>
        <span style={{
          fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', color: chipColor,
...
const HeroPanel = ({ data, heroVis, coursesAbove, coursesBelow, totalSkippable }: {
  data: any; heroVis: boolean; coursesAbove: number; coursesBelow: number; totalSkippable: number;
}) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-label)', letterSpacing: '0.15em', marginBottom: 6, fontWeight: 700 }}>
              OVERALL ATTENDANCE
            </div>
            <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-display)', lineHeight: 1, color: 'var(--text-primary)' }}>
              {data.totalClassesAttended}
              <span style={{ color: 'var(--text-muted)', fontSize: 16, fontWeight: 400 }}>
                {' '}/ {data.totalClassesHeld}
              </span>
            </div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-tertiary)', marginTop: 5 }}>
              classes attended
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 'clamp(16px,3vw,28px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(12px,2vw,20px)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-label)', letterSpacing: '0.18em', marginBottom: 8, fontWeight: 700 }}>
              OVERALL ATTENDANCE
            </div>
            <div style={{ fontSize: 'clamp(22px,4vw,34px)', fontWeight: 700, fontFamily: 'var(--font-display)', lineHeight: 1, letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
              {data.totalClassesAttended}
              <span style={{ color: 'var(--text-muted)', fontSize: 'clamp(13px,2vw,18px)', fontWeight: 400 }}>
                {' '}/ {data.totalClassesHeld}
              </span>
            </div>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', color: 'var(--text-primary)', fontFamily: 'var(--font-display)', position: 'relative' }}>
      ...
      <main style={{ position: 'relative', zIndex: 1, maxWidth: 1320, margin: '0 auto', padding: `clamp(80px,10vw,100px) ${pad} 0` }}>
        ...
        <div style={{
          ...
          <h1 style={{ margin: 0, fontSize: 'clamp(30px,5vw,52px)', fontWeight: 700, fontFamily: 'var(--font-display)', letterSpacing: '-0.025em', lineHeight: 1.15, color: 'var(--text-primary)' }}>
            {fullName}
          </h1>
          {data.regNumber && (
      ...
      <footer style={{
        ...
        marginTop: 'clamp(40px,6vw,64px)', borderTop: '1px solid var(--border-subtle)',
      }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', letterSpacing: '-0.02em', marginBottom: 6 }}>OASYS</div>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.05em', lineHeight: 2, textAlign: 'right', maxWidth: 400 }}>
            <div style={{ color: 'var(--accent-amber)', fontWeight: 700, letterSpacing: '0.1em', marginBottom: 4 }}>⚠ DISCLAIMER</div>
